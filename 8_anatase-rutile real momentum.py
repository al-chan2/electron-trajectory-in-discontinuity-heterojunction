...
# --- 8. EXACT TRANSMISSION (Ip Correlation) ---
...
# Export for Origin Heatmap (Graph 26 / Eq 32 visualization)
df_heatmap = pd.DataFrame(heatmap_matrix, columns=np.round(x, 2))
df_heatmap.insert(0, 'Time', np.round(time_array, 3))
df_heatmap.to_csv('Phase_Dependent_Mass_Heatmap.csv', index=False)